...
async function main() {
  const contract = await hre.ethers.deployContract("Swisstronik", ["Hello Swisstronik from ItsJoey!!"]);
...
async function main() {
  ...
  const messageToSet = "Hello Swisstronik from ItsJoey!!";
